...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-5 left-4 right-4 z-50 mx-auto max-w-md bg-white/80 border border-slate-200/50 rounded-[24px] shadow-[0_15px_40px_rgba(0,0,0,0.08)] backdrop-blur-2xl h-16 flex items-center justify-between px-5">
      ...
      >
        <LayoutDashboard className={`size-[20px] ${pathname === "/" ? "fill-emerald-100/55" : ""}`} strokeWidth={pathname === "/" ? 2.5 : 1.75} />
        <span className="text-[10px] font-medium">Home</span>
        {pathname === "/" && (
          <span className="absolute -bottom-1 size-1 rounded-full bg-emerald-500 animate-pulse" />
        )}
      </Link>
      ...
      >
        <History className={`size-[20px]`} strokeWidth={pathname.startsWith("/history") ? 2.5 : 1.75} />
      ...
      <div className="relative -top-3.5">
        <Link
          href="/scan"
          className="flex items-center justify-center size-12 rounded-[18px] bg-emerald-600 text-white shadow-[0_8px_20px_rgba(16,185,129,0.35)] active:scale-90 transition-all hover:bg-emerald-500 hover:shadow-[0_8px_25px_rgba(16,185,129,0.45)] group cursor-pointer"
        >
          <Camera className="size-[21px] transition-transform duration-300 group-hover:rotate-6" strokeWidth={2.25} />
        </Link>
      </div>
      ...
      >
        <MessageCircle className={`size-[20px] ${pathname === "/chat" ? "fill-emerald-100/55" : ""}`} strokeWidth={pathname === "/chat" ? 2.5 : 1.75} />
      ...
      >
        <UserRound className={`size-[20px] ${pathname === "/settings" ? "fill-emerald-100/55" : ""}`} strokeWidth={pathname === "/settings" ? 2.5 : 1.75} />
